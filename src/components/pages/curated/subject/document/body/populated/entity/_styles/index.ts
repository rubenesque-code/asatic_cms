import tw from "twin.macro";

export const $EntityContainer = tw.div`relative px-sm py-md min-h-[250px] border-b`;

export const $statusContainer = tw`mb-sm`;

export const $imageContainer = tw`relative w-[250px] aspect-ratio[16 / 9] float-left mr-sm`;

export const $Title = tw.h3`text-2xl font-serif-eng`;

export const $SubTitle = tw.div`text-xl font-serif-eng flex gap-xs mt-xxs flex-wrap`;

export const $Authors = tw.div`flex items-center`;

export const $Text = tw.div`text-base font-serif-eng mt-xs`;
