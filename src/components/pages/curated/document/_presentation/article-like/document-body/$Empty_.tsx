...
export function $DocumentBodyEmpty_({
  addSectionPopover,
  entityType,
}: {
  addSectionPopover: (button: ReactElement) => ReactElement;
  ...
  return (
    <div css={[tw`mt-md font-sans`]}>
      ...
      <div css={[tw`flex items-center gap-xxs mt-xs`]}>
        <p css={[tw`text-gray-600`]}>Get started with the {entityType}.</p>
        {addSectionPopover(<AddSectionButton />)}
      </div>
    </div>
  );
}
